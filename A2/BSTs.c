...
BST_Node *find_successor(BST_Node *right_child_node)
{
    ...
    successor = right_child_node -> right;
...
// void BST_index_change(BST_Node *root)
// {
...
//     BST_index_change(root -> left);

//     root -> index = 1 - root -> index;

//     BST_index_change(root -> right);
    
// }

// void BST_key_change(BST_Node *root)
// {
...
//     BST_key_change(root -> left);

//     root -> key = (10.0 * root -> bar) + root -> index;

//     BST_key_change(root -> right);

// }

// double BST_bar_reverse(BST_Node *root, int i)
// {
//     double array[100][2];
//     for (int i = 0; i < 100; i++){
//         for (int j = 0; j < 2; j++){
...
//     reverse_bars(array, count);
//     return array;

// }

// void reverse_bars(double array[100][2], int count)
// {
//     int i;
//     double temp1;
//     double temp2;
...
BST_Node *reverseSong(BST_Node *root)
{
    ...
    // BST_index_change(root);
    // double array[100][2] = BST_bar_reverse(root, 0);
    // int count = 0;
    // while (array[count][0] != 0){
    //     count += 1;
    // }
...
void insert_new_nodes (BST_Node *root, int semitones, double time_shift)
{
    if (root == NULL){
        return;
    }
    ...
    double new_freq;
    double new_index;
    new_freq = find_new_freq(note_freq, semitones, root -> freq); // find new freq function
    new_index = find_new_index (root -> index, time_shift); // find new index function
    if (new_freq != 0.0 && new_index != 0.0){
        BST_Node *new_note = NULL;
        new_note = newBST_Node(new_freq, root -> bar, new_index);
        BST_Node *new_root = NULL;
        new_root = BST_insert(root, new_note);
    }
