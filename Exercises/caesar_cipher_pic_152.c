  ...
  while (plaintext[count] != '\0'){
    for (int i = 0; i < 50; i++){
      for (int j = 0; j < 50; j++){
        ...
        plaintext[count] = plaintext[count] + passPic[i][j];
        if (plaintext[count] > 255){
          plaintext[count] = plaintext[count] - 255;
        }
        count += 1;
      }
    }
...
void caesar_decipher(unsigned char cryptotext[MAX_STR_LEN], unsigned char passPic[50][50])
{
 ...
 // TO DO: Implement this function
  ...
  int count = 0;
  while (cryptotext[count] != '\0'){
    for (int i = 0; i < 50; i++){
      for (int j = 0; j < 50; j++){
        if (cryptotext[count] == '\0'){
          break;
        }
        cryptotext[count] = cryptotext[count] - passPic[i][j];
        if (cryptotext[count] <= 0){
            cryptotext[count] = 255 + cryptotext[count];
        }
        count += 1;
      }
    }
