...
void wordSwapper(char *dest, char *src)
{
  ...
  while (src != end_string_pointer)
  {
    char *endp = src;

    while ((*src != ' ') && (src != end_string_pointer))
    {
      src -= 1;
  ...
  while (dest < (dest_start_point + MAX_STR_LEN + 1))
  {
    *dest = '\0';
    dest += 1;
  }
